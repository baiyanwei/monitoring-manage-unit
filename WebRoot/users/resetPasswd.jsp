		...
		<div style="padding:10px 0 10px 60px">
		    <form id="ff" name="ff" action="resetPasswd.action" method="post" onsubmit="return submitForm();">
	    		...
	    		<tr>
	    			...
	    			<td><input id="newpasswd" class="easyui-validatebox" type="password" missingMessage="请输入新密码,长度要大于${minlength},至少包括1个数字，1个字母，1个特殊字符" name="newpasswd" data-options="required:true" onblur="checknewPasswd();"></input><div id="d2" style="color:red"></div></td>
	    		</tr>
  ...
  <script>
		function submitForm(){
			...
			if(newpasswd.value!=rnewpasswd.value){
				alert("重复密码与新密码不相同，请重新填写！");
				return false;
			}
			if(newpasswd.value==oldpasswd.value){
				alert("新旧密码不能相同，请重新填写！");
				return false;
			}
		...
		function checknewPasswd(){
			var newpasswd=document.getElementById("newpasswd");
			$.post('checkNewPasswd.action', {newpasswd:newpasswd.value},function(data){
